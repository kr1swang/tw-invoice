...
export default function PrizeDesc({ prizeList = [], winNumbers = {} }) {
  const dataSource = prizeList.map((item) => ({
    ...item,
    number: Array.isArray(winNumbers[item.mapping]) ? winNumbers[item.mapping].join(', ') : winNumbers[item.mapping]
  }))

  return (
    <Card className={'prizeDesc'}>
      <h3 className={'title'}>{winNumbers.cTitle || '中獎號碼'}</h3>
      <Table dataSource={dataSource} rowKey={'prize'} scroll={{ y: '40vh' }} pagination={false} size={'small'}>
